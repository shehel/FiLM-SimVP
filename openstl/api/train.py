...
class BaseExperiment(object):
    ...
    def vali(self, logger, epoch):
        ...
        for i in [10, 50, 100, 150]:
            ...
            for pixel in self.vali_loader.dataset.pixel_list:
                ...
                for idx, quantile in enumerate(self.vali_loader.dataset.quantiles[::2]):
                    plt.plot(x_values, results['preds'][i, idx*2, :, 0, y, x], label=f"Preds_q{quantile} at {pixel}")
    ...
    def test(self, task_name, csv_path=None):
        ...
        try:
            for key, value in results.items():
                if key not in ['inputs', 'trues', 'preds', 'masks']:
                    if isinstance(value, list) and all(isinstance(item, torch.Tensor) for item in value):
                        print(f"{key}:")
                        for i, tensor in enumerate(value):
                            print(tensor.tolist())  # Convert tensor to list for full precision
                    elif isinstance(value, torch.Tensor):
                        print(f"{key}: {value.tolist()}")  # Convert tensor to list for full precision
                    elif isinstance(value, float):
                        print(f"{key}: {value:.20f}")  # Print float with 20 decimal places
                    else:
                        print(f"{key}: {value}")
        ...
        if csv_path:
            save_results_to_csv(results, task_name, csv_path)

        return 0#eval_res['mse']

    def inference(self, task_name, best_model=True, csv_path=None):
...
def save_results_to_csv(results, task_name, csv_path):
    ...
    #         if isinstance(value, list) and all(isinstance(item, torch.Tensor) for item in value):
    #             for i, tensor in enumerate(value):
    #                 data[f"{key}_{i}"] = tensor.item() if tensor.numel() == 1 else tensor.tolist()
    #         elif isinstance(value, torch.Tensor):
    #             data[key] = value.item() if value.numel() == 1 else value.tolist()
    #         elif isinstance(value, float):
    #             data[key] = f"{value:.20f}"
    #         else:
    #             data[key] = str(value)
    ...
    #     fieldnames = ['task_name'] + list(data.keys())[1:]  # Exclude 'task_name' from data keys
    ...
    #     # If the task doesn't exist, append a new row
    #     if not task_exists:
    #         writer.writerow(data)
    #     else:
    #         # If the task exists, update the existing row
    #         rows = []
    #         with open(csv_path, 'r') as csvfile:
    #             reader = csv.DictReader(csvfile)
    #             for row in reader:
