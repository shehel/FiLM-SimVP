...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=384, N_S=4, N_T=4, model_type='gSTA',
        ...
        self.dec1 = Decoder(hid_S, C, N_S, spatio_kernel_dec, act_inplace=act_inplace)
        self.dec2 = Decoder(hid_S, C, N_S, spatio_kernel_dec, act_inplace=act_inplace)
    ...
    def forward(self, inputs):
        ...
        Y = self.dec(hid, skip)
        Y_lo = self.dec1(hid, skip)
        Y_hi = self.dec2(hid, skip)
        Y = torch.stack([Y_lo, Y, Y_hi], dim=1)
...
class Decoder(nn.Module):
    ...
    def forward(self, hid, enc1=None, condi=None):
        ...
        #Y = self.readout(Y)
        #Y = torch.cat((#self.lower2(Y).unsqueeze(1),
                       #self.lower1(Y).unsqueeze(1),
         #              self.readout(Y).unsqueeze(1),
                       #self.upper1(Y).unsqueeze(1),
                       #self.upper2(Y).unsqueeze(1)), dim=1)
        Y = self.readout(Y)#.unsqueeze(1)
        return Y
