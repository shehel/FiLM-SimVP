...
class SimVPQFiLM_Model(nn.Module):
    r"""SimVP Model
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=384, N_S=4, N_T=4, model_type='gSTA',
        ...
        super(SimVPQFiLM_Model, self).__init__()
        T, C, H, W = in_shape  # T is pre_seq_length
        H, W = int(H / 2**(N_S/2)), int(W / 2**(N_S/2))  # downsample 1 / 2**(N_S/2)
        act_inplace = False
        # self.gen1 = FilmGen(1, [16,32], 32)
        # self.gen2 = FilmGen(1, [64,256], 256)
    ...
    def forward(self, inputs):
        ...
        embed, skip = self.enc(x, quantiles)
    ...
    def __init__(self, C_in, C_hid, N_S, spatio_kernel, act_inplace=True):
        ...
        self.enc = nn.Sequential(
              ConvSC(C_in, C_hid, spatio_kernel, downsampling=samplings[0],
                     act_inplace=act_inplace, filmed=True, indepndent_film=True),
            *[ConvSC(C_hid, C_hid, spatio_kernel, downsampling=s,
                     act_inplace=act_inplace, filmed=True, indepndent_film=True) for s in samplings[1:]]
        )
...
class MetaBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, input_resolution=None, model_type=None,
                 mlp_ratio=8., drop=0.0, drop_path=0.0, layer_i=0, filmed=True):
        ...
        if model_type == 'gsta':
            self.block = GASubBlock(
                ...
                drop=drop, drop_path=drop_path, act_layer=nn.GELU, filmed=filmed, indepndent_film=True)
    ...
    def __init__(self, channel_in, channel_hid, N2,
        ...
        for i in range(1, N2-1):
            enc_layers.append(MetaBlock(
                ...
                mlp_ratio, drop, drop_path=dpr[i], layer_i=i, filmed=True, indepndent_film=True))
        # upsample
        enc_layers.append(MetaBlock(
