...
class FilmGen(nn.Module):
    """ Multi-layer Perceptron to generate gamma and beta using separate networks. """
    def __init__(self, input_size, layer_sizes, output_size):
        ...
        for size in layer_sizes:
            gamma_layers.append(nn.Linear(current_size, size))
            gamma_layers.append(nn.LeakyReLU())
            current_size = size
        gamma_layers.append(nn.Linear(current_size, output_size))
        self.gamma_network = nn.Sequential(*gamma_layers)

        # Beta network
        beta_layers = []
        current_size = input_size
        for size in layer_sizes:
            beta_layers.append(nn.Linear(current_size, size))
            beta_layers.append(nn.LeakyReLU())
            current_size = size
        beta_layers.append(nn.Linear(current_size, output_size))
        self.beta_network = nn.Sequential(*beta_layers)

    def forward(self, x):
        try:
            gamma = self.gamma_network(x)
            beta = self.beta_network(x)
        except:
            pdb.set_trace()
        return gamma, beta
...
class BasicConv2d(nn.Module):
    ...
    def __init__(self,
                 ...
                 act_inplace=True,
                 filmed=False,
                 independent_film=False):
        super(BasicConv2d, self).__init__()
        self.filmed = filmed
        self.independent_film = independent_film
        ...
        if self.filmed:
            if self.independent_film:
                self.film_gen1 = FilmGen(1, [16],out_channels)
            self.norm1 = nn.GroupNorm(2, out_channels, affine=False)
            self.norm2 = FiLM()
        else:
    ...
    def forward(self, x, condi=None):
        ...
        if self.act_norm:
            if self.filmed:
                if self.independent_film:
                    gamma1,beta1 = self.film_gen1(condi)
                    # Repeat each element 4 times in sequence
                    gamma1 = gamma1.repeat_interleave(4, dim=0)
                    beta1 = beta1.repeat_interleave(4, dim=0)
                else:
                    gamma1,beta1 = condi[0],condi[1]
                try:
                    y = self.act(self.norm2(self.norm1(y), gamma1, beta1))
                except:
                    pdb.set_trace()
            else:
...
class ConvSC(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv = BasicConv2d(C_in, C_out, kernel_size=kernel_size, stride=stride,
                                ...
                                act_norm=act_norm, act_inplace=act_inplace, filmed=filmed, independent_film=independent_film)
...
class GASubBlock(nn.Module):
    ...
    def __init__(self, dim, kernel_size=21, mlp_ratio=4.,
                 drop=0., drop_path=0.1, init_value=1e-2, act_layer=nn.GELU, filmed=False, independent_film=False):
        super().__init__()
        self.filmed = filmed
        self.independent_film = independent_film
        if filmed:
            if independent_film:
               self.film_gen1 = FilmGen(1, [32],dim)
            self.norm1 = nn.BatchNorm2d(dim, affine=False)
            self.norm2 = FiLM()
        else:
        ...
        if filmed:
            if independent_film:
                self.film_gen2 = FilmGen(1, [32],dim)
            self.norm3 = nn.BatchNorm2d(dim, affine=False)
            self.norm4 = FiLM()
        else:
    ...
    def forward(self, x, condi=None):
        if self.filmed:
            if self.independent_film:
                gamma1,beta1 = self.film_gen1(condi)
                gamma2,beta2 = self.film_gen2(condi)
            else:
                gamma1,beta1 = condi[0],condi[1]
                gamma2,beta2 = condi[0],condi[1]
            x = x + self.drop_path(
                    self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * self.attn(self.norm2(self.norm1(x), gamma1, beta1)))
            x = x + self.drop_path(
                self.layer_scale_2.unsqueeze(-1).unsqueeze(-1) * self.mlp(self.norm4(self.norm3(x), gamma2, beta2)))
        else:
            x = x + self.drop_path(
                self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * self.attn(self.norm1(x)))
            x = x + self.drop_path(
                self.layer_scale_2.unsqueeze(-1).unsqueeze(-1) * self.mlp(self.norm3(x)))
        return x
